...
plugins/avif/
  CMakeLists.txt          identity (AVIF <version>, priority 10), avif_core / avif_adapter /
                          avif_composition, pvdkit_add_plugin(avif ...), the license ports
  ...
  fixtures/               sample files + SOURCES.md (origin, license, expected values)
...
with the plugin, static distribution documents, dependency licenses and package manifest.
...
Fixtures live in `fixtures/` and are documented in `fixtures/SOURCES.md` (origin, license, what
...
Signature Edits unrestricted-use license); `fixtures/LIBAVIF_DATA_README.md` is the upstream
