...
- `decodeFrame`: `avifDecoderNthImage`; `avifRGBImageSetDefaults`; `depth = 8`;
  ...
  `avifImageYUVToRGB`. No allocation inside the adapter; `detail::checkDestination` refuses a
...
selection, EXIF orientation (AVIF says `irot`/`imir` win), 16-bit output. libavif rejects
