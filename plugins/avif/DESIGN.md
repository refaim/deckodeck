...
- `DecoderFactory::create()`: `avifDecoderCreate`, set `maxThreads`,
  ...
  `masteringPeakNits`). libavif 1.4.2 does not expose parsed `mdcv`, so absent CLLI uses the shared
  pipeline's 1000-nit PQ fallback. Maps `avifResult` to `ErrorCode`
...
- Grid images, progressive files, 10/12-bit sources, image sequences: handled by libavif. The
  ...
  applied: PictureView ignores them and pvdkit does not have a CMS yet.
...
maps, progressive preview rendering, layered/`a1lx` selection, EXIF orientation (AVIF says
`irot`/`imir` win). libavif rejects
