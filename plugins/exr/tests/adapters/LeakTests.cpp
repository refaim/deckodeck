// Unit-level leak gate of the OpenEXR adapter and the composed plugin, in-process (no DLL): a
// decoder created, used and destroyed N times on every fixture - and refused N times on every
// negative fixture - returns every heap block and every handle, the Core's contexts, chunk
// pipelines and scratch buffers included (tests/support/LeakCheck.hpp).
...
namespace
{
    ...
    constexpr DecoderOptions kOptions{4, false, std::uint64_t{16384} * 16384, 32768, true};
    ...
    // Splits the fixtures the way the factory sees them: parsed or refused. The large corpus files
    // (800x800 and more) are left out of the cycling to keep the N = 200 budget on the plain presets.
    struct Split
    {
    ...
    Split split(std::vector<Fixture> fixtures)
    {
        ...
        for (auto &fixture : fixtures) {
            ...
            if (parsed && fixture.bytes.size() > 64 * 1024) {
                continue;
            }
...
TEST_CASE("leak: a decoder created, used and destroyed N times on every fixture returns everything")
{
    ...
    const auto report = measureLeaks(
        "exr::Decoder create/decode/destroy", fixtures.parsed.size(), kIterations, [&](const std::size_t i) {
            ...
            const std::uint32_t pitch = meta.width * 8U;
            std::vector<std::byte> pixels(static_cast<std::size_t>(pitch) * meta.height);
            REQUIRE((*decoder)->frameTiming(0).has_value());
            REQUIRE((*decoder)->decodeFrame(0, PixelFormat::Bgra64, pixels, pitch).has_value());
            // Error paths on a live decoder: a frame out of range and a destination too small.
            CHECK_FALSE((*decoder)->frameTiming(1).has_value());
            CHECK_FALSE((*decoder)->decodeFrame(1, PixelFormat::Bgra64, pixels, pitch).has_value());
            CHECK_FALSE((*decoder)
                            ->decodeFrame(0, PixelFormat::Bgra64, std::span{pixels}.first(pixels.size() - 1), pitch)
                            .has_value());
        });
...
TEST_CASE("leak: a decoder refused N times on every negative fixture returns everything")
{
    ...
    pvdkit::exr::DecoderFactory factory;
    const auto report =
        measureLeaks("exr::DecoderFactory refusals", fixtures.refused.size(), kIterations, [&](const std::size_t i) {
            ...
            CHECK_FALSE(
                factory.create(std::span{fixture.bytes}.first(std::min<std::size_t>(3, fixture.bytes.size())), kOptions)
                    .has_value());
        });
    requireNoLeak(report);
}
