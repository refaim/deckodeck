...
inline constexpr std::array allErrorCodes{
    ...
    core::ErrorCode::InvalidTransform,
...
struct FakeState {
  ...
  ImageInfo imageInfo{2, true, "AVIF session format", "AV1 session compression",
                      "session comments"};
...
class FakeSession final : public IFileSession {
 public:
  ...
  ~FakeSession() override { --state_.liveSessions; }
  ...
  [[nodiscard]] core::Result<DecodedPage> decodePage(const std::uint32_t page,
...
class FakePlugin final : public IPlugin {
 public:
  explicit FakePlugin(FakeState& state) : state_{state} { ++state_.livePlugins; }
  ~FakePlugin() override { --state_.livePlugins; }
