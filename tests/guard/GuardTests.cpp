...
struct IncludeIndex {
  std::map<std::string, std::set<std::string>> headersByRoot;
};
...
void appendOwnershipViolations(std::vector<Violation>& violations, const std::string_view normalizedPath,
  ...
  for (const auto& [root, headers] : index.headersByRoot) {
  ...
  for (const auto& included : capturedIncludes(includeCode, regexes.anyInclude)) {
    const auto resolvesIn = [&](const bool allowedRoots) {
      return std::ranges::any_of(index.headersByRoot, [&](const auto& entry) {
...
[[nodiscard]] IncludeIndex indexHeaders(const std::vector<std::filesystem::path>& roots) {
  ...
  for (const auto& root : roots) {
    auto& headers = index.headersByRoot[normalizedLowerPath(std::filesystem::absolute(root))];
...
[[nodiscard]] IncludeIndex fakeIndex() {
  ...
  IncludeIndex index;
  index.headersByRoot[root("project/src")] = {"core/error.hpp", "pvd/types.hpp", "adapters/win/utf8.hpp"};
  index.headersByRoot[root("project/plugins/avif/src")] = {"adapters/avif/decoder.hpp", "core/describe.hpp"};
  index.headersByRoot[root("project/plugins/rpgmvp/src")] = {"adapters/spng/decoder.hpp", "core/describe.hpp"};
  return index;
}
...
TEST_CASE("foreign headers are restricted to adapters and Exports") {
  for (const auto include : {std::string_view{"#include <windows.h>"},
                             ...
                             std::string_view{"#include \"AVIF/AVIF.H\""},
                             std::string_view{"#include <dav1d/dav1d.h>"}}) {
    CAPTURE(include);
...
TEST_CASE("core may include only the shared pvd boundary contracts, in the shared and the plugin tree") {
  for (const auto path : {std::string_view{"project/src/core/Sample.cpp"},
    ...
    for (const auto header : {std::string_view{"Shim.hpp"}, std::string_view{"ContextHandle.hpp"},
                              ...
                              std::string_view{"PluginConstants.hpp"}, std::string_view{"PluginIdentity.hpp"}}) {
      CAPTURE(header);
...
TEST_CASE("shared code never includes a plugin header") {
  ...
  for (const auto path : {std::string_view{"project/src/core/FileSession.cpp"},
                          std::string_view{"project/src/pvd/Exports.cpp"},
                          std::string_view{"project/src/adapters/win/FileSource.cpp"}}) {
    CAPTURE(path);
