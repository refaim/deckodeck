...
std::vector<unsigned> pixelIds(const pvd::DecodedPage &page,
                               const std::uint32_t width,
                               const std::uint32_t bytesPerPixel) {
  std::vector<unsigned> result;
  const auto height =
      static_cast<std::uint32_t>(page.pixels.size() / page.pitchBytes);
  for (std::uint32_t y = 0; y < height; ++y) {
    for (std::uint32_t x = 0; x < width; ++x) {
      result.push_back(std::to_integer<unsigned>(
          page.pixels[static_cast<std::size_t>(y) * page.pitchBytes +
                      x * bytesPerPixel]));
    }
  }
  return result;
...
TEST_CASE("FileSession exposes its image information and rejects out-of-range "
  ...
  FileSession session(nullptr, decoder(imageMeta, state), info,
                      test::options());
...
TEST_CASE("pageInfo reports still dimensions depth and no frame timing") {
  ...
  FileSession session(nullptr, decoder(imageMeta, state),
                      test::imageInfo(imageMeta), test::options());
...
TEST_CASE("decodePage emits BGR24 pixels and all progress steps") {
  ...
  const pvd::Progress progress{
      [&reports](const std::uint32_t step, const std::uint32_t steps) {
  ...
  CHECK(reports == std::vector<std::pair<std::uint32_t, std::uint32_t>>{
                       {0, 3}, {1, 3}, {2, 3}});
...
TEST_CASE("decodePage aborts at each progress step without retaining a page") {
  for (std::uint32_t abortStep = 0; abortStep < 3; ++abortStep) {
    ...
    const pvd::Progress progress{
        [&](const std::uint32_t step, const std::uint32_t steps) {
          CHECK(steps == 3);
...
TEST_CASE("decodePage enforces the coded pixel limit before decoding") {
  ...
  FileSession session(nullptr, decoder(imageMeta, state),
                      test::imageInfo(imageMeta), test::options(5));
...
TEST_CASE("FileSession retains two pages and frees them in either order") {
  for (const bool reverse : {false, true}) {
    ...
    CHECK(pixelIds(*second, 3, 3) ==
          std::vector<unsigned>{101, 102, 103, 104, 105, 106});
    // The first view stays valid and untouched until freePage.
    CHECK(std::vector(first->pixels.begin(), first->pixels.end()) ==
          firstPixels);
    CHECK(pixelIds(*first, 3, 3) == std::vector<unsigned>{1, 2, 3, 4, 5, 6});
...
TEST_CASE("FileSession destruction releases file data decoder and outstanding "
  ...
  const auto imageMeta = test::meta(3, 2, false, 8, 2, true);
  {
    auto fileData = std::make_unique<test::FakeFileData>(
        std::vector<std::byte>{std::byte{1}, std::byte{2}}, dataDestructions);
    FileSession session(std::move(fileData), decoder(imageMeta, state),
                        test::imageInfo(imageMeta), test::options());
