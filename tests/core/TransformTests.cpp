...
TestImage image(const std::uint32_t width, const std::uint32_t height,
                const std::uint32_t bytesPerPixel,
                const std::initializer_list<unsigned> values,
                const std::uint32_t padding = 0) {
  const auto pitch = width * bytesPerPixel + padding;
  TestImage result{
      std::vector<std::byte>(static_cast<std::size_t>(pitch) * height), width,
      height, bytesPerPixel, pitch};
  auto value = values.begin();
  for (std::uint32_t y = 0; y < height; ++y) {
    for (std::uint32_t x = 0; x < width; ++x) {
      for (std::uint32_t channel = 0; channel < bytesPerPixel; ++channel) {
        const auto offset =
            static_cast<std::size_t>(y) * pitch + x * bytesPerPixel + channel;
        result.pixels[offset] =
            std::byte{static_cast<unsigned char>(*value + channel * 16)};
      }
      ++value;
    }
  }
...
std::vector<unsigned> pixelIds(const PixelView view) {
  ...
  for (std::uint32_t y = 0; y < view.height; ++y) {
    for (std::uint32_t x = 0; x < view.width; ++x) {
      result.push_back(std::to_integer<unsigned>(
          view.pixels[static_cast<std::size_t>(y) * view.pitchBytes +
                      x * view.bytesPerPixel]));
    }
  }
  return result;
}

void checkPixels(const PixelBuffer &actual,
                 const std::initializer_list<unsigned> expected,
...
ImageMeta metaWith(const std::uint32_t width, const std::uint32_t height,
...
TEST_CASE("validatedCrop accepts each image corner") {
  for (const auto &[rect, expected] :
       std::array{std::pair{CropRect{0, 0, 2, 2}, CropRect{0, 0, 2, 2}},
...
TEST_CASE("validatedCrop rejects empty and outside rectangles") {
  for (const auto rect :
       {CropRect{0, 0, 0, 1}, CropRect{0, 0, 1, 0}, CropRect{3, 0, 1, 1},
...
PixelBuffer cropOnly(const PixelView view, const CropRect &rect) {
  auto result = Transform::apply(Transforms{rect, 0, std::nullopt}, view,
...
PixelBuffer rotateOnly(const PixelView view, const std::uint8_t angle) {
  auto result = Transform::apply(Transforms{std::nullopt, angle, std::nullopt},
                                 view, std::numeric_limits<std::uint64_t>::max());
  REQUIRE(result.has_value());
  return std::move(*result);
}

PixelBuffer mirrorOnly(const PixelView view, const MirrorAxis axis) {
  auto result = Transform::apply(Transforms{std::nullopt, 0, axis}, view,
                                 std::numeric_limits<std::uint64_t>::max());
  REQUIRE(result.has_value());
  return std::move(*result);
}
...
TEST_CASE("crop copies all channels from padded rows at each corner") {
  ...
  checkPixels(cropOnly(source.view(), CropRect{0, 0, 2, 2}), {1, 2, 4, 5}, 2,
              2, 3);
  checkPixels(cropOnly(source.view(), CropRect{1, 0, 2, 2}), {2, 3, 5, 6}, 2,
              2, 3);
  checkPixels(cropOnly(source.view(), CropRect{0, 1, 2, 2}), {4, 5, 7, 8}, 2,
              2, 3);
  checkPixels(cropOnly(source.view(), CropRect{1, 1, 2, 2}), {5, 6, 8, 9}, 2,
              2, 3);
}
...
TEST_CASE("mirror applies both axes to 2x3 and 3x2 BGRA images") {
  ...
  checkPixels(mirrorOnly(tall.view(), MirrorAxis::TopBottom),
              {5, 6, 3, 4, 1, 2}, 2, 3, 4);
  checkPixels(mirrorOnly(tall.view(), MirrorAxis::LeftRight),
              {2, 1, 4, 3, 6, 5}, 2, 3, 4);

  const auto wide = image(3, 2, 4, {1, 2, 3, 4, 5, 6}, 1);
  checkPixels(mirrorOnly(wide.view(), MirrorAxis::TopBottom),
              {4, 5, 6, 1, 2, 3}, 3, 2, 4);
  checkPixels(mirrorOnly(wide.view(), MirrorAxis::LeftRight),
              {3, 2, 1, 6, 5, 4}, 3, 2, 4);
}

TEST_CASE("displaySize crops before swapping odd rotations") {
  Transforms transforms;
  CHECK(Transform::displaySize(metaWith(5, 3, transforms)) ==
        std::pair{5U, 3U});
...
TEST_CASE("hasTransforms recognizes each transformative property") {
  ...
  CHECK(Transform::hasTransforms(
      Transforms{CropRect{0, 0, 1, 1}, 0, std::nullopt}));
  CHECK(Transform::hasTransforms(Transforms{std::nullopt, 1, std::nullopt}));
  CHECK(Transform::hasTransforms(
      Transforms{std::nullopt, 0, MirrorAxis::LeftRight}));
  CHECK(Transform::hasTransforms(
      Transforms{CropRect{0, 0, 1, 1}, 1, MirrorAxis::TopBottom}));
}
...
TEST_CASE("apply supports each property independently") {
  ...
  auto cropped = Transform::apply(
      Transforms{CropRect{1, 0, 2, 2}, 0, std::nullopt}, source.view(), 4);
  REQUIRE(cropped.has_value());
  checkPixels(*cropped, {2, 3, 5, 6}, 2, 2, 3);

  auto rotated = Transform::apply(Transforms{std::nullopt, 1, std::nullopt},
                                  source.view(), 6);
  REQUIRE(rotated.has_value());
  checkPixels(*rotated, {3, 6, 2, 5, 1, 4}, 2, 3, 3);

  auto mirrored = Transform::apply(
      Transforms{std::nullopt, 0, MirrorAxis::TopBottom}, source.view(), 6);
...
TEST_CASE("apply propagates invalid crops and output size limits") {
  ...
  const auto invalid = Transform::apply(
      Transforms{CropRect{1, 2, 2, 2}, 0, std::nullopt}, source.view(), 6);
  REQUIRE_FALSE(invalid.has_value());
  CHECK(invalid.error().code == ErrorCode::InvalidTransform);

  const auto tooLarge = Transform::apply(
      Transforms{std::nullopt, 1, std::nullopt}, source.view(), 5);
  REQUIRE_FALSE(tooLarge.has_value());
  CHECK(tooLarge.error().code == ErrorCode::TooLarge);

  const auto croppedTooLarge = Transform::apply(
      Transforms{CropRect{0, 0, 2, 3}, 0, std::nullopt}, source.view(), 5);
  REQUIRE_FALSE(croppedTooLarge.has_value());
  CHECK(croppedTooLarge.error().code == ErrorCode::TooLarge);

  const auto mirroredTooLarge = Transform::apply(
      Transforms{std::nullopt, 0, MirrorAxis::LeftRight}, source.view(), 5);
