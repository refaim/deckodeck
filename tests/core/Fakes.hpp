...
namespace pvdkit::core::test
{
    ...
    inline Error error(const ErrorCode code, std::string detail = "scripted failure")
    {
    ...
    class FakeDecoder final : public IDecoder
    {
      public:
        ...
        [[nodiscard]] Result<void> decodeFrame(const std::uint32_t frame, const pvd::PixelFormat format,
            ...
            for (std::uint32_t y = 0; y < meta_.height; ++y) {
                for (std::uint32_t x = 0; x < meta_.width; ++x) {
                    ...
                    for (std::uint32_t channel = 0; channel < channels; ++channel) {
                        const auto offset =
                            static_cast<std::size_t>(y) * pitchBytes + static_cast<std::size_t>(x) * channels + channel;
                        destination[offset] = std::byte{static_cast<unsigned char>(id + channel * 16)};
                    }
                }
            }
            return {};
    ...
    class FakeFileSource final : public IFileSource
    {
      public:
        ...
        [[nodiscard]] Result<std::unique_ptr<IFileData>> open(const std::string_view utf8Path) override
        {
            auto &state = state_.get();
            ++state.openCalls;
            ...
            return std::unique_ptr<IFileData>{std::make_unique<FakeFileData>(state.fileBytes, state.dataDestructions)};
        }
    ...
    class FakeDecoderFactory final : public IDecoderFactory
    {
      public:
        ...
        [[nodiscard]] bool recognises(const std::span<const std::byte> head) const override
        {
        ...
        [[nodiscard]] Result<std::unique_ptr<IDecoder>> create(const std::span<const std::byte> file,
                                                               const DecoderOptions &options) override
        {
            ...
            return std::unique_ptr<IDecoder>{std::make_unique<FakeDecoder>(meta_, decoderState_.get())};
        }
    ...
    struct DescriberState
    {
        ...
        ImageDescription description{"Fake format", "Fake compression", "fake description"};
    ...
    class FakeImageDescriber final : public IImageDescriber
    {
      public:
        ...
        [[nodiscard]] ImageDescription describe(const ImageMeta &imageMeta) const override
        {
            auto &state = state_.get();
            ++state.describeCalls;
    ...
    inline ImageMeta meta(const std::uint32_t width = 3, const std::uint32_t height = 2, const bool hasAlpha = false,
                          const std::uint8_t depth = 8, const std::uint32_t frameCount = 1, const bool animated = false,
    ...
    {
        return pvd::ImageInfo{imageMeta.frameCount, imageMeta.animated, "Fake format", "Fake compression",
                              "test image"};
    }
