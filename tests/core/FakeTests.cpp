...
TEST_CASE("CodecPlugin::open propagates an exception from recognises "
  ...
  CHECK_THROWS_AS(static_cast<void>(
                      plugin.open(pvd::OpenRequest{"throw.avif", 1, head})),
...
TEST_CASE("FileSession::pageInfo propagates an exception from frameTiming and "
  ...
  const auto imageMeta = test::meta(3, 2, false, 8, 2, true);
  {
    auto fileData = std::make_unique<test::FakeFileData>(
        std::vector<std::byte>{std::byte{1}}, dataDestructions);
    ...
    CHECK_THROWS_AS(static_cast<void>(session.pageInfo(1)),
                    test::FrameTimingException);
...
TEST_CASE("FileSession::decodePage propagates an exception from decodeFrame "
  ...
  const auto imageMeta = test::meta(3, 2, false, 8, 2, true);
  {
    ...
    CHECK_THROWS_AS(static_cast<void>(session.decodePage(1, pvd::Progress{})),
                    test::DecodeFrameException);
...
TEST_CASE("FileSession::decodePage propagates an exception from the Progress "
  ...
  for (std::uint32_t throwStep = 0; throwStep < 3; ++throwStep) {
    ...
    const auto imageMeta = test::meta(3, 2, false, 8, 2, true);
    {
      ...
      const pvd::Progress throwingProgress{
      ...
      CHECK_THROWS_AS(
          static_cast<void>(session.decodePage(1, throwingProgress)),
          std::logic_error);
