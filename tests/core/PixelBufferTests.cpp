...
namespace pvdkit::core
{
    namespace
    {
        ...
        TEST_CASE("PixelBuffer creates tightly packed writable storage and a const view")
        {
        ...
        {
            const auto created = PixelBuffer::create(std::numeric_limits<std::uint32_t>::max(), 1, 4,
        ...
        TEST_CASE("PixelBuffer rejects a byte count the address space cannot hold")
        {
            ...
            if constexpr (sizeof(std::size_t) == 4) {
                const auto created = PixelBuffer::create(65536, 65536, 1, std::numeric_limits<std::uint64_t>::max());

                REQUIRE_FALSE(created.has_value());
                CHECK(created.error().code == ErrorCode::TooLarge);
                CHECK(created.error().detail == "pixel buffer size (4294967296) does not fit in 32 bits");
            }
        }
        ...
        TEST_CASE("PixelBuffer rejects overflowing storage arithmetic")
        {
            ...
            const auto created =
                PixelBuffer::create(maximum, maximum, maximum, std::numeric_limits<std::uint64_t>::max());
