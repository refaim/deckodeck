...
struct Harness {
  ...
  explicit Harness(ImageMeta imageMeta = test::meta(),
                   const DecoderOptions options = test::options())
...
TEST_CASE("CodecPlugin rejects a failed signature before opening, parsing or "
  ...
  const auto opened =
      harness.plugin.open(pvd::OpenRequest{"ignored.bin", 100, head});
...
TEST_CASE("CodecPlugin memory mode parses the whole head without using the "
  ...
  auto opened =
      harness.plugin.open(pvd::OpenRequest{"archive-entry.bin", 0, wholeFile});
...
TEST_CASE("CodecPlugin file mode parses opened file bytes and retains their "
          "owner") {
  Harness harness;
  harness.fileState.fileBytes = {std::byte{9}, std::byte{8}, std::byte{7},
                                 std::byte{6}};
  const std::vector head{std::byte{1}, std::byte{2}};

  auto opened =
      harness.plugin.open(pvd::OpenRequest{"C:/images/photo.bin", 4, head});
...
TEST_CASE("CodecPlugin passes through file source errors without creating a "
          "decoder") {
  Harness harness;
  harness.fileState.openError =
      test::error(ErrorCode::FileOpenFailed, "missing file");
  const std::vector head{std::byte{1}};

  const auto opened =
      harness.plugin.open(pvd::OpenRequest{"missing.bin", 10, head});
...
TEST_CASE("CodecPlugin passes through factory errors and releases opened file "
  ...
  harness.factoryState.createError =
...
TEST_CASE("canonical source, factory and describer interfaces destroy "
  ...
  std::unique_ptr<IDecoderFactory> factory =
      std::make_unique<test::FakeDecoderFactory>(factoryState, decoderState,
                                                 test::meta());
  std::unique_ptr<IImageDescriber> describer =
      std::make_unique<test::FakeImageDescriber>(describerState);
