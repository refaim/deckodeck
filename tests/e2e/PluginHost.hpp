...
class PluginLibrary {
 public:
  [[nodiscard]] static std::expected<PluginLibrary, std::string> load(
      const std::filesystem::path& path);

  [[nodiscard]] const PluginExports& exports() const noexcept { return exports_; }
...
enum class OpenMode { Disk, Memory };
...
struct DecodedPage {
  ...
  [[nodiscard]] std::uint32_t bytesPerPixel() const noexcept { return decode.nBPP / 8; }
...
[[nodiscard]] std::optional<DecodedPage> decodePage(const PluginExports& exports, void* context,
