...
struct FixtureExpectation {
  ...
  std::uint32_t width;   // page size as the host sees it: after clap/irot
  std::uint32_t height;
  std::uint32_t pages;
  std::uint32_t pageBpp;  // informational nBPP of pvdInfoPage: depth x channels
...
std::string text(const char* value) { return value == nullptr ? std::string{"<null>"} : std::string{value}; }
...
WorkerOutcome decodeOnWorker(const PluginExports& exports, const FixtureFile& file) {
  ...
  if (exports.fileOpen(file.utf8Path.c_str(), static_cast<INT64>(file.bytes.size()),
                       reinterpret_cast<const BYTE*>(file.bytes.data()), headSize, &info, &context) == FALSE) {
...
TEST_CASE("every accepted fixture decodes identically from disk and from memory") {
  ...
  for (const auto& expected : kFixtures) {
...
TEST_CASE("a fixture libavif refuses is refused consistently in both modes without crashing") {
  const auto plugin = loadInitializedPlugin();
  const auto& exports = plugin.exports();
  const auto file = readFixture("clap_irot_imir_non_essential.avif");
  ...
  for (std::uint32_t y = 0; y < 512; y += 3) {
    for (std::uint32_t x = 0; x < 256; x += 3) {
      ...
      for (std::size_t channel = 0; channel < 4; ++channel) {
        worst = std::max(worst, std::abs(static_cast<int>(pixelA[channel]) - static_cast<int>(pixelB[channel])));
      }
...
TEST_CASE("non-AVIF, garbage, truncated, empty and short inputs are rejected") {
  ...
  for (const auto name : {"not_avif.png", "not_avif.bmp", "garbage.bin", "truncated.avif"}) {
    CAPTURE(name);
