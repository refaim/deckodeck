...
FileSession::FileSession(std::unique_ptr<IFileData> fileData,
                         std::unique_ptr<IDecoder> decoder,
                         pvd::ImageInfo imageInfo, DecoderOptions options)
    : fileData_(std::move(fileData)), decoder_(std::move(decoder)),
      imageInfo_(std::move(imageInfo)), options_(options) {}

FileSession::~FileSession() = default;

const pvd::ImageInfo &FileSession::imageInfo() const { return imageInfo_; }
...
Result<pvd::PageInfo> FileSession::pageInfo(const std::uint32_t page) const {
  ...
  return pvd::PageInfo{width, height,
                       static_cast<std::uint32_t>(meta.depth) * channels,
                       frameTimeMs};
}

Result<pvd::DecodedPage>
FileSession::decodePage(const std::uint32_t page,
  ...
  auto created = PixelBuffer::create(meta.width, meta.height, bytesPerPixel,
                                     options_.maxPixels);
  if (!created) {
  ...
  const auto decoded =
      decoder_->decodeFrame(page, format, buffer.bytes(), buffer.pitchBytes());
  if (!decoded) {
  ...
  if (Transform::hasTransforms(meta.transforms)) {
    auto transformed =
        Transform::apply(meta.transforms, buffer.view(), options_.maxPixels);
    if (!transformed) {
