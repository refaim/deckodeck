...
/// factory (adapter over the codec library) and the describer (the plugin's host-facing words).
class CodecPlugin final : public pvd::IPlugin {
public:
  CodecPlugin(IFileSource &fileSource, IDecoderFactory &decoderFactory,
              const IImageDescriber &describer, DecoderOptions options,
