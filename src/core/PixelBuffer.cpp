...
namespace pvdkit::core
{
    ...
    Result<PixelBuffer> PixelBuffer::create(const std::uint32_t width, const std::uint32_t height,
                                            const std::uint32_t bytesPerPixel, const std::uint64_t maxPixels)
    {
        const auto pixelCount = static_cast<std::uint64_t>(width) * height;
        if (pixelCount > maxPixels) {
            return std::unexpected(tooLarge("pixel count exceeds the configured limit"));
        }

        const auto pitch = static_cast<std::uint64_t>(width) * bytesPerPixel;
        if (height != 0 && pitch > std::numeric_limits<std::uint64_t>::max() / height) {
        ...
        return narrow<std::size_t>(pitch * height, "pixel buffer size").transform([&](const std::size_t size) {
    ...
    PixelBuffer::PixelBuffer(std::unique_ptr<std::byte[]> bytes, const std::size_t size, const std::uint32_t width,
                             const std::uint32_t height, const std::uint32_t bytesPerPixel,
                             const std::uint32_t pitchBytes) noexcept
        : bytes_(std::move(bytes)), size_(size), width_(width), height_(height), bytesPerPixel_(bytesPerPixel),
    ...
    std::uint32_t PixelBuffer::pitchBytes() const noexcept
    {
        return pitchBytes_;
    }
