...
// function can fail).
Result<PixelBuffer> transform(const PixelView view, const CropRect &rect,
                              const std::uint8_t angle,
                              const std::optional<MirrorAxis> axis,
  ...
  auto created = PixelBuffer::create(outputWidth, outputHeight,
                                     view.bytesPerPixel, maxPixels);
  if (!created) {
  ...
  for (std::uint32_t y = 0; y < outputHeight; ++y) {
    for (std::uint32_t x = 0; x < outputWidth; ++x) {
      ...
      const auto rotatedX =
          axis == MirrorAxis::LeftRight ? outputWidth - 1 - x : x;
      const auto rotatedY =
          axis == MirrorAxis::TopBottom ? outputHeight - 1 - y : y;
      ...
      const auto sourceOffset =
          (static_cast<std::size_t>(rect.y) + croppedY) * view.pitchBytes +
          (static_cast<std::size_t>(rect.x) + croppedX) * view.bytesPerPixel;
      const auto destinationOffset =
          static_cast<std::size_t>(y) * output.pitchBytes() +
          static_cast<std::size_t>(x) * view.bytesPerPixel;
      std::copy_n(view.pixels.begin() + sourceOffset, view.bytesPerPixel,
                  output.bytes().begin() + destinationOffset);
    }
  }
  return created;
}
...
Result<CropRect> validatedCrop(const CropRect &rect,
  ...
  const bool valid = (rect.width != 0) & (rect.height != 0) &
                     (right <= imageWidth) & (bottom <= imageHeight);
  if (!valid) {
    return std::unexpected(Error{ErrorCode::InvalidTransform,
...
bool hasTransforms(const Transforms &transforms) noexcept {
  return transforms.clap.has_value() || transforms.irotAngle != 0 ||
         transforms.imir.has_value();
}

Result<PixelBuffer> apply(const Transforms &transforms, const PixelView view,
                          const std::uint64_t maxPixels) {
  ...
  return transform(view, rect, transforms.irotAngle, transforms.imir,
                   maxPixels);
}
