...
namespace pvdkit::core
{
    ...
    {
      public:
        [[nodiscard]] static Result<PixelBuffer> create(std::uint32_t width, std::uint32_t height,
                                                        std::uint32_t bytesPerPixel, std::uint64_t maxPixels);
      ...
      private:
        PixelBuffer(std::unique_ptr<std::byte[]> bytes, std::size_t size, std::uint32_t width, std::uint32_t height,
