...
std::string_view chromaName(const ChromaFormat chroma) {
  ...
  constexpr std::array names{
      std::string_view{"YUV 4:4:4"}, std::string_view{"YUV 4:2:2"},
...
std::string describe(const ImageMeta &meta) {
  ...
  append(description, "CICP " + std::to_string(meta.cicp.primaries) + "/" +
                          std::to_string(meta.cicp.transfer) + "/" +
                          std::to_string(meta.cicp.matrix) +
                          std::string{cicpSuffix(meta.cicp)});

  if (meta.hasAlpha) {
    append(description,
           meta.alphaPremultiplied ? "premultiplied alpha" : "straight alpha");
  }
  if (meta.animated) {
  ...
  if (meta.transforms.imir) {
    append(description, *meta.transforms.imir == MirrorAxis::TopBottom
                            ? "imir top-bottom"
                            : "imir left-right");
  }
  return description;
