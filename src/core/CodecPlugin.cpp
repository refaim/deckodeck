...
CodecPlugin::CodecPlugin(IFileSource &fileSource,
                         IDecoderFactory &decoderFactory,
                         const IImageDescriber &describer,
                         const DecoderOptions options,
                         pvd::PluginInfo pluginInfo)
    : fileSource_(fileSource), decoderFactory_(decoderFactory),
      describer_(describer), options_(options),
      pluginInfo_(std::move(pluginInfo)) {}

const pvd::PluginInfo &CodecPlugin::info() const { return pluginInfo_; }

Result<std::unique_ptr<pvd::IFileSession>>
CodecPlugin::open(const pvd::OpenRequest &request) {
  if (!decoderFactory_.recognises(request.head)) {
    return std::unexpected(Error{ErrorCode::NotRecognised,
                                 "input does not carry a recognised signature"});
  }
  ...
  pvd::ImageInfo imageInfo{meta.frameCount, meta.animated,
                           std::move(description.formatName),
                           std::move(description.compression),
                           std::move(description.comments)};
