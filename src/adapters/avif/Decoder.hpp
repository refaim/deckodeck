...
[[nodiscard]] core::Result<core::Transforms> transforms(const avifImage& image,
...
[[nodiscard]] core::Result<void> checkDestination(const core::ImageMeta& meta,
                                                   pvd::PixelFormat format, std::size_t dstSize,
...
[[nodiscard]] avifRGBImage rgbTarget(const avifImage& image, pvd::PixelFormat format,
                                     std::span<std::byte> dst, std::uint32_t pitchBytes,
                                     int maxThreads);
...
class Decoder final : public core::IDecoder {
 public:
  ...
  [[nodiscard]] core::Result<core::FrameTiming> frameTiming(
      std::uint32_t frame) const override;
