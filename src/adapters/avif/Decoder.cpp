...
core::Error resultError(const avifResult result, const core::ErrorCode code,
...
core::Result<core::ChromaFormat> detail::chromaFormat(const avifPixelFormat format) {
  switch (format) {
    ...
    case AVIF_PIXEL_FORMAT_COUNT:
      return std::unexpected(core::Error{core::ErrorCode::ParseFailed,
                                         avifResultToString(AVIF_RESULT_NO_YUV_FORMAT_SELECTED)});
  }
  return std::unexpected(core::Error{core::ErrorCode::Internal,
                                     avifResultToString(AVIF_RESULT_UNKNOWN_ERROR)});
}

core::Result<core::Transforms> detail::transforms(const avifImage& image,
                                                   avifDiagnostics& diagnostics) {
  core::Transforms result{};
  if ((image.transformFlags & AVIF_TRANSFORM_CLAP) != 0) {
    avifCropRect crop{};
    if (avifCropRectFromCleanApertureBox(&crop, &image.clap, image.width, image.height,
                                         &diagnostics) == AVIF_FALSE) {
      return std::unexpected(resultError(AVIF_RESULT_INVALID_ARGUMENT,
                                         core::ErrorCode::InvalidTransform, diagnostics));
    }
  ...
  if ((image.transformFlags & AVIF_TRANSFORM_IMIR) != 0) {
    ...
    result.imir = image.imir.axis == 0 ? core::MirrorAxis::TopBottom
                                      : core::MirrorAxis::LeftRight;
  }
  return result;
}
...
core::Result<void> detail::checkDestination(const core::ImageMeta& meta,
                                            const pvd::PixelFormat format,
                                            const std::size_t dstSize,
                                            const std::uint32_t pitchBytes) {
  ...
  if (dstSize < required) {
    return std::unexpected(core::Error{core::ErrorCode::Internal,
                                       "caller buffer is smaller than pitch multiplied by height"});
  }
  return {};
}
...
core::Result<core::ImageMeta> imageMeta(avifDecoder& decoder) {
  ...
  return detail::chromaFormat(image.yuvFormat)
      .and_then([&](const core::ChromaFormat chroma) {
        return detail::transforms(image, decoder.diag)
            .transform([&](core::Transforms normalizedTransforms) {
              return core::ImageMeta{
                  ...
                  core::Cicp{image.colorPrimaries, image.transferCharacteristics,
                             image.matrixCoefficients, image.yuvRange == AVIF_RANGE_FULL},
                  static_cast<std::uint32_t>(decoder.imageCount),
                  decoder.imageCount > 1,
                  std::move(normalizedTransforms),
                  image.icc.size != 0,
                  image.exif.size != 0,
                  image.xmp.size != 0,
              };
...
void DecoderDestroy::operator()(avifDecoder* decoder) const noexcept { avifDecoderDestroy(decoder); }
...
const core::ImageMeta& Decoder::meta() const { return meta_; }
...
core::Result<core::FrameTiming> Decoder::frameTiming(const std::uint32_t frame) const {
  ...
  return detail::checkedResult(result, errorCodeForResult(result), decoder_->diag)
      .transform([&] { return core::FrameTiming{detail::durationMilliseconds(timing.duration)}; });
}
...
core::Result<std::unique_ptr<core::IDecoder>> DecoderFactory::create(
    const std::span<const std::byte> file, const core::DecoderOptions& options) {
  DecoderHandle decoder = detail::requireDecoder(DecoderHandle{avifDecoderCreate()});

  const int maxThreads = static_cast<int>(std::min<unsigned>(
      options.maxThreads, static_cast<unsigned>(std::numeric_limits<int>::max())));
  ...
  decoder->imageSizeLimit = static_cast<std::uint32_t>(std::clamp<std::uint64_t>(
      options.maxPixels, 1, static_cast<std::uint64_t>(AVIF_DEFAULT_IMAGE_SIZE_LIMIT)));
  decoder->imageDimensionLimit = options.maxDimension;

  const avifResult ioResult = avifDecoderSetIOMemory(
      decoder.get(), reinterpret_cast<const std::uint8_t*>(file.data()), file.size());
  return detail::checkedResult(ioResult, errorCodeForResult(ioResult), decoder->diag)
      .and_then([&]() -> core::Result<std::unique_ptr<core::IDecoder>> {
        ...
        if (parseResult != AVIF_RESULT_OK) {
          ...
          const core::ErrorCode code = isSizeLimitDiagnostic(diagnostic)
                                           ? core::ErrorCode::TooLarge
                                           : errorCodeForResult(parseResult);
        ...
        return imageMeta(*decoder).transform([&](core::ImageMeta meta) {
          return std::unique_ptr<core::IDecoder>{std::make_unique<Decoder>(
              Decoder::Key{}, std::move(decoder), std::move(meta), maxThreads)};
        });
      });
}
