...
namespace pvdkit::win
{
    ...
    core::Result<std::wstring> utf8ToWide(const std::string_view utf8)
    {
        ...
        return detail::utf8Length(utf8.size()).and_then([&](const int utf8Size) -> core::Result<std::wstring> {
            // Both calls pass utf8Size, which is utf8.size() narrowed by utf8Length, next to data();
            // clang-tidy only recognises a literal size() argument, hence the two NOLINTs.
            const int required = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS,
                                                     utf8.data(), // NOLINT(*-stringview-data-usage)
                                                     utf8Size, nullptr, 0);
            if (required == 0) {
    ...
    {
        if (path.size() < MAX_PATH || path.starts_with(kExtendedPrefix) || path.starts_with(kDevicePrefix)) {
