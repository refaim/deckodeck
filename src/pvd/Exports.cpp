...
namespace
{
    ...
    class ProcessState final
    {
      public:
        ...
        [[nodiscard]] pvdkit::pvd::Shim &shim() noexcept
        {
            return shim_;
        }
                                      ...
                                      const UINT32 headSize, pvdInfoImage *output, void **context)
{
    return pvdkit::pvd::guarded(
        [&] {
            return processState ? processState->shim().fileOpen(fileName, fileSize, head, headSize, output, context)
                                : FALSE;
        },
        FALSE);
}
...
{
    return pvdkit::pvd::guarded(
        [&] { return processState ? processState->shim().pageInfo(context, page, output) : FALSE; }, FALSE);
}
                                        ...
                                        const pvdDecodeCallback callback, void *callbackContext)
{
    return pvdkit::pvd::guarded(
        [&] {
            return processState ? processState->shim().pageDecode(context, page, output, callback, callbackContext)
                                : FALSE;
        },
        FALSE);
}
