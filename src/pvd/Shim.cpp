...
BOOL Shim::fileOpen(const char* fileName, const INT64 fileSize, const BYTE* head,
                    const UINT32 headSize, pvdInfoImage* output, void** context) noexcept {
  return guarded(
      [&] {
        if (fileName == nullptr) {
          return BOOL{0};
        ...
        *context = nullptr;
        ...
        auto opened = plugin_.open(OpenRequest{std::string_view{fileName},
                                                static_cast<std::uint64_t>(fileSize), bytes});
        if (!opened) {
          return BOOL{0};
        }
...
BOOL Shim::pageDecode(void* context, const UINT32 page, pvdInfoDecode* output,
                      const pvdDecodeCallback callback, void* callbackContext) noexcept {
  return guarded(
      [&] {
        if (output == nullptr) {
          return BOOL{0};
        ...
        const Progress progress = callback == nullptr
                                      ? Progress{}
                                      : Progress{[callback, callbackContext](
        ...
        output->pImage =
            const_cast<BYTE*>(reinterpret_cast<const BYTE*>(decoded->pixels.data()));
...
void Shim::pageFree(void* context, pvdInfoDecode* decoded) noexcept {
  guarded([&] {
    ...
    const auto pixels = std::span<const std::byte>{
        reinterpret_cast<const std::byte*>(decoded->pImage), std::size_t{0}};
